...
const initialSubs = [
  ...
  },
  {
    ...
    status: "active",
    nextDate: "May 10",
    logo: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/Amazon_Prime_Video_logo_%282024%29.svg/960px-Amazon_Prime_Video_logo_%282024%29.svg.png",
    color: "#ff9900",
    since: "Feb 2019",
  },
  {
    ...
    status: "active",
    nextDate: "May 14",
    logo: "https://upload.wikimedia.org/wikipedia/commons/4/42/Adobe_Acrobat_DC_logo_2020.svg",
    color: "#ff0000",
    since: "Jun 2022",
  },
  {
    ...
    status: "active",
    nextDate: "May 18",
    logo: "https://logowik.com/content/uploads/images/youtube-premium8029.logowik.com.webp",
    color: "#c94c4c",
    since: "Nov 2021",
  },
  {
  ...
  },
  {
    ...
    status: "active",
    nextDate: "May 1",
    logo: "https://ui-avatars.com/api/?name=Gym+Membership&background=2a7d4f&color=fff",
    color: "#2a7d4f",
    since: "Jan 2024",
  },
  {
  ...
  },
  {
    ...
    status: "active",
    nextDate: "May 9",
    logo: "https://cdn.prod.website-files.com/66c503d081b2f012369fc5d2/674000d6c0a42d41f8c331be_dropbox-2-logo-png-transparent.png",
    color: "#0061ff",
    since: "Apr 2021",
  },
];
...
export default function BillManager() {
  ...
  const upcomingCount = bills.filter(
    (b) => b.status === "due" || b.status === "upcoming",
  ).length;
    ...
    if (addType === "bill") {
      ...
      setSubs((prev) => [
        ...prev,
        {
          ...item,
          status: "active",
          logo: form.icon || "https://ui-avatars.com/api/?name=" + form.name,
          nextDate: `May ${form.dueDay || 1}`,
          since: `${today.toLocaleDateString("en-US", { month: "short", year: "numeric" })}`,
        },
      ]);
  ...
  const markPaid = (id) => {
    setBills((prev) =>
      prev.map((b) =>
        b.id === id ? { ...b, status: "paid", lastPaid: `Apr ${b.dueDay}` } : b,
      ),
    );
  ...
  const filteredSubs =
    filter === "All" ? subs : subs.filter((s) => s.category === filter);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      <PageHeader
        ...
        actions={
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
              onClick={() => {
                setAddType("bill");
                setShowAdd(true);
              }}
            >
              <Icons.AddBill /> {t("Add Bill")}
            </button>
            <button
              className="px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-medium rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
              onClick={() => {
                setAddType("sub");
                setShowAdd(true);
              }}
            >
              <Icons.AddSub /> {t("Add Subscription")}
            </button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={upcomingCount}
          sub="within 30 days"
          color={upcomingCount > 3 ? "rose" : "emerald"}
        />
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto animate-fadeIn shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
                {addType === "bill" ? t("Add Bill") : t("Add Subscription")}
              </h3>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  value={form.freq}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, freq: e.target.value }))
                  }
                >
                  {["Monthly", "Quarterly", "Annual", "Weekly"].map((f) => (
              ...
              <div>
                ...
                >
                  {[
                    ...new Set(
                      [...billCats, ...subCats].filter((c) => c !== "All"),
                    ),
                  ].map((c) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {addType === "bill"
                    ? t("Icon Name (e.g. Home, Zap)")
                    : t("Logo URL")}
                </label>
                <input
                  className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500 transition-colors"
                  placeholder={addType === "bill" ? "Home" : "https://logo.url"}
                  value={form.icon}
                  onChange={(e) =>
              ...
              <div className="sm:col-span-2 flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                <div
                  onClick={() =>
                    setForm((p) => ({ ...p, autoPay: !p.autoPay }))
                  }
                  className={`w-9 h-5 rounded-full cursor-pointer transition-all ${form.autoPay ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`}
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-all mt-0.5 ${form.autoPay ? "ml-4" : "ml-0.5"}`}
                  />
                </div>
                <span className="text-sm text-gray-700 dark:text-gray-300 flex items-center gap-1">
                  <Icons.AutoPay /> {t("Auto-pay enabled")}
                </span>
              </div>
            </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 flex-wrap">
              {upcomingBills.slice(0, 4).map((b, i) => (
                ...
                >
                  {b.icon} {b.name} — due{" "}
                  {b.dueDay < today.getDate() ? "overdue" : `May ${b.dueDay}`} ·{" "}
                  {fmt(b.amount)}
                </span>
              ))}
            </div>
              ...
              <tbody>
                {filteredBills.map((b) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      {b.autoPay ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 flex items-center gap-1 w-fit">
                          <Icons.AutoPay /> Auto
                        </span>
                      ) : (
                    ...
                    <td
                      className={`p-4 text-right text-sm font-semibold ${b.status === "paid" ? "text-gray-500 dark:text-gray-500" : "text-gray-900 dark:text-gray-200"}`}
                    >
                      {fmtFull(b.amount)}
                    </td>
                    <td className="p-4">
                      <div className="flex gap-1">
                        ...
                        <button
                          onClick={() => {
                            ...
                            toast.success("Deleted", "Bill removed.");
                          }}
      ...
      {tab === "subscriptions" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSubs.map((s) => (
            ...
            >
              <div
                className={`absolute top-0 left-0 w-1 h-full ${s.status === "active" ? "bg-emerald-500" : "bg-gray-400 dark:bg-gray-600"}`}
              />
              <div>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span
                    className={`px-2 py-0.5 text-xs rounded-full ${s.status === "active" ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" : "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-500"}`}
                  >
                    {s.status}
                  </span>
                </div>
                <div className="flex justify-between items-center mb-3">
                  <div>
                    <div
                      className={`text-2xl font-bold ${s.status === "active" ? "text-gray-900 dark:text-gray-200" : "text-gray-500 dark:text-gray-500"}`}
                    >
                      {fmtFull(s.amount)}
                      <span className="text-xs text-gray-500 dark:text-gray-500 ml-1">
                        /{s.freq === "Monthly" ? "mo" : "yr"}
                      </span>
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-500">
                      Next: {s.nextDate}
                  ...
                  <div className="text-right">
                    ...
                    <div className="font-semibold text-gray-700 dark:text-gray-300 text-sm">
                      {fmt(s.amount * (s.freq === "Monthly" ? 12 : 1))}
                    </div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-1.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition-all flex items-center justify-center gap-1"
                  >
                  ...
                  <button
                    onClick={() => {
                      ...
                      toast.success("Deleted", "Subscription removed.");
                    }}
                    className="px-3 py-1.5 text-xs font-medium bg-rose-100 dark:bg-rose-900/30 hover:bg-rose-200 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-400 rounded-lg transition-all"
                  >
                    <Icons.Cancel />
                  </button>
                </div>
