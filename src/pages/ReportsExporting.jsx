...
function generateDataForYear(yearStr) {
  ...
  const monthlyData = [
    ...
    return {
      ...
      expenses,
      savings,
      investments: Math.round(1800 * baseMulti),
      networth: Math.round(100000 + (year - 2021) * 30000 + i * savings),
    };
  });
  ...
  const yearlyData = [2021, 2022, 2023, 2024, 2025].map((y) => {
    ...
    return {
      year: y.toString(),
      income: Math.round(84000 * yMulti),
      expenses: Math.round(63000 * yMulti),
      savings: Math.round(21000 * yMulti),
      taxPaid: Math.round(14200 * yMulti),
      networth: Math.round(98000 + (y - 2021) * 20000),
    };
  });
  ...
  const taxData = {
    grossIncome,
    adjustments: Math.round(-27150 * baseMulti),
    agi: Math.round(73650 * baseMulti),
    deductions: Math.round(-14600 * baseMulti),
    taxableIncome: Math.round(59050 * baseMulti),
    fedTax: Math.round(8142 * baseMulti),
    stateTax: Math.round(3251 * baseMulti),
    ficaTax: Math.round(7711 * baseMulti),
    totalTax: Math.round(19104 * baseMulti),
    effectiveRate: 18.96,
  };

  const expensesData = [
    { name: "Housing", value: Math.round(22200 * baseMulti) },
    { name: "Food", value: Math.round(7080 * baseMulti) },
    { name: "Transport", value: Math.round(4440 * baseMulti) },
    { name: "Insurance", value: Math.round(6300 * baseMulti) },
    { name: "Entertainment", value: Math.round(3480 * baseMulti) },
    { name: "Subscriptions", value: Math.round(2700 * baseMulti) },
    { name: "Healthcare", value: Math.round(1740 * baseMulti) },
    { name: "Other", value: Math.round(14460 * baseMulti) },
  ];
...
export default function ReportsExporting() {
  ...
  const [customReports, setCustomReports] = useLocalStorage(
    "wp_custom_reports",
  ...
  const dynamicData = React.useMemo(
    () => generateDataForYear(reportYear),
    [reportYear],
  );
  ...
  const handleExport = (format, name) => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        "Export Complete",
        `"${name}" downloaded as ${format.toUpperCase()}`,
      );
    }, 800);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      <PageHeader
        title={t("Reports & Exporting")}
        subtitle={t("Financial reports, tax summaries and data exports")}
        actions={
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
            {[
              ...
              <div
                key={i}
                className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5"
              >
                <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-500 mb-2">
                  {t(s.label)}
                </div>
                <div className={`text-2xl md:text-3xl font-bold ${s.color}`}>
                  {s.value}
                </div>
              </div>
            ))}
              ...
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {dynamicData.expensesData.map((_, i) => (
                ...
                <tbody>
                  {dynamicData.monthlyData.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-sm font-semibold text-gray-900 dark:text-gray-300">
                        {fmt(r.networth)}
                      </td>
                      <td className="p-4">
                        <span className="px-2 py-1 text-xs rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">
                          {((r.savings / r.income) * 100).toFixed(1)}%
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      {tab === "yearly" && (
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
            {[
              {
                label: `${reportYear} Income`,
                value: fmt(100800),
                color: "text-emerald-600 dark:text-emerald-400",
              },
              {
                label: `${reportYear} Expenses`,
                value: fmt(62400),
                color: "text-rose-600 dark:text-rose-400",
              },
              {
                label: `${reportYear} Savings`,
                value: fmt(38400),
                color: "text-amber-600 dark:text-amber-400",
              },
              {
                label: `${reportYear} Tax Paid`,
                value: fmt(18200),
              ...
              <table className="w-full">
                ...
                <tbody>
                  {dynamicData.yearlyData.map((r, i) => (
                    <tr
                      key={i}
                      className={`border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors ${r.year === reportYear ? "bg-emerald-50 dark:bg-emerald-900/10" : ""}`}
                    >
                      <td className="p-4 text-sm font-medium text-gray-900 dark:text-gray-300">
                        {r.year}{" "}
                        {r.year === reportYear && (
                          <span className="ml-2 px-1.5 py-0.5 text-xs rounded bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">
                            current
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-sm text-purple-600 dark:text-purple-400">
                        {fmt(r.taxPaid)}
                      </td>
                      <td className="p-4 text-sm font-semibold text-gray-900 dark:text-gray-300">
                        {fmt(r.networth)}
      ...
      {tab === "tax" && (
        <div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
            <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <div className="text-sm font-semibold text-gray-900 dark:text-gray-200 mb-4">
                Tax-Ready Summary — {reportYear}
              </div>
              <div className="space-y-0">
                {[
                  {
                    label: "Gross Income",
                    value: dynamicData.taxData.grossIncome,
                    color: "text-emerald-600 dark:text-emerald-400",
                    bold: false,
                  },
                  {
                    label: "Pre-tax Adjustments",
                    value: dynamicData.taxData.adjustments,
                    color: "text-emerald-600 dark:text-emerald-400",
                    bold: false,
                  },
                  {
                    label: "Adjusted Gross Income",
                    value: dynamicData.taxData.agi,
                    color: "text-amber-600 dark:text-amber-400",
                    bold: true,
                  },
                  null,
                  {
                    ...
                    value: dynamicData.taxData.deductions,
                    color: "text-emerald-600 dark:text-emerald-400",
                    bold: false,
                  },
                  {
                    label: "Taxable Income",
                    value: dynamicData.taxData.taxableIncome,
                    color: "text-amber-600 dark:text-amber-400",
                    bold: true,
                  },
                  null,
                  {
                    ...
                    value: dynamicData.taxData.fedTax,
                    color: "text-rose-600 dark:text-rose-400",
                    bold: false,
                  },
                  {
                    label: "State Tax (est.)",
                    value: dynamicData.taxData.stateTax,
                    color: "text-rose-600 dark:text-rose-400",
                    bold: false,
                  },
                  {
                    label: "FICA / Payroll Tax",
                    value: dynamicData.taxData.ficaTax,
                    color: "text-pink-600 dark:text-pink-400",
                    bold: false,
                  },
                  {
                    label: "TOTAL TAX OWED",
                    value: dynamicData.taxData.totalTax,
                    color: "text-rose-600 dark:text-rose-400",
                    bold: true,
                  },
                  null,
                  {
                    ...
                    value:
                      dynamicData.taxData.grossIncome -
                      dynamicData.taxData.totalTax,
                    color: "text-emerald-600 dark:text-emerald-400",
                    bold: true,
                  },
                  {
                    label: "Effective Tax Rate",
                    value: `${dynamicData.taxData.effectiveRate}%`,
                    color: "text-amber-600 dark:text-amber-400",
                    bold: false,
                    isStr: true,
                  },
                  ...
                  item === null ? (
                    ...
                    <div
                      key={i}
                      className={`flex justify-between py-2 ${item.bold ? "text-sm" : "text-xs"}`}
                    >
                      <span className="text-gray-600 dark:text-gray-500">
                        {item.label}
                      </span>
            ...
            <div className="space-y-5">
                ...
                {[
                  ...
                  <div key={i} className="mb-3">
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span
                        className={`font-semibold ${d.limit && d.used >= d.limit ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}
                      >
                        {d.limit
                    ...
                    {d.limit && (
                      <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(d.used / d.limit) * 100}%`,
                            background:
                              d.used >= d.limit ? "#10b981" : "#f59e0b",
                          }}
                        />
                      </div>
                    )}
              ...
              <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                ...
                {[
                  {
                    date: "Jan 31",
                    event: "W-2s must be sent by employers",
                    done: true,
                  },
                  {
                    date: "Apr 15",
                    event: "Federal tax return deadline",
                    done: true,
                  },
                  {
                    date: "Jun 15",
                  ...
                  <div
                    key={i}
                    className="flex gap-2 items-center py-2 border-b border-gray-100 dark:border-gray-700/50 last:border-0"
                  >
                    <span
                      className={`text-xs font-bold min-w-[50px] ${d.done ? "text-gray-400 dark:text-gray-500" : "text-amber-600 dark:text-amber-400"}`}
                    >
                      {d.date}
                    </span>
                    <span
                      className={`text-xs flex-1 ${d.done ? "text-gray-400 dark:text-gray-500 line-through" : "text-gray-700 dark:text-gray-300"}`}
                    >
                      {d.event}
                    </span>
                    {d.done && (
          ...
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="E.g., My Q3 Budget"
                  className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCustomForm((p) => ({ ...p, type: e.target.value }))
                  }
                >
                  {[
                    "Income Summary",
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCustomForm((p) => ({ ...p, range: e.target.value }))
                  }
                >
                  {[
                    "This Month",
              ...
              <div>
                ...
                <select
                  className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500"
                  value={customForm.groupBy}
                  onChange={(e) =>
                    setCustomForm((p) => ({ ...p, groupBy: e.target.value }))
                  }
                >
                  {["Category", "Month", "Week", "Account", "Payee"].map(
                    (g) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCustomForm((p) => ({ ...p, format: e.target.value }))
                  }
                >
                  {[
                    "PDF Report",
            ...
            <div className="flex gap-3">
              <button
                onClick={() =>
                  handleExport(
                    customForm.format.includes("CSV")
                      ? "CSV"
                      : customForm.format.includes("JSON")
                        ? "JSON"
                        : customForm.format.includes("PDF")
                          ? "PDF"
                          : "Excel",
                    customForm.name || "Custom Report",
                  )
                }
              ...
              <button
                onClick={() => {
                  ...
                  const existing = customReports.find(
                    (r) => r.name === customForm.name,
                  );
                  if (existing) {
                    setCustomReports((prev) =>
                      prev.map((r) =>
                        r.name === customForm.name
                          ? { ...r, ...customForm }
                          : r,
                      ),
                    );
          ...
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {customReports.map((t, i) => (
                <div
                  key={t.id}
                  className="bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700 p-3 hover:border-gray-300 dark:hover:border-gray-600 transition-all cursor-pointer"
                >
                  ...
                  <div className="flex gap-2 mt-3">
                    <button
                      onClick={() =>
                        handleExport(
                          t.format?.includes("CSV")
                            ? "CSV"
                            : t.format?.includes("JSON")
                              ? "JSON"
                              : t.format?.includes("PDF")
                                ? "PDF"
                                : "Excel",
                          t.name,
                        )
                      }
                      className="flex-1 px-2 py-1 text-xs font-medium bg-emerald-100 dark:bg-emerald-600/20 hover:bg-emerald-200 dark:hover:bg-emerald-600/40 text-emerald-700 dark:text-emerald-400 rounded-md transition-all flex items-center justify-center gap-1"
                    >
                      <Icons.Run /> Run
                    </button>
                    <button
                      onClick={() =>
                        setCustomForm({
                          name: t.name,
                          type: t.type || "Income Summary",
                          range: t.range || "This Month",
                          groupBy: t.groupBy || "Category",
                          format: t.format || "PDF Report",
                        })
                      }
                      className="px-2 py-1 text-xs font-medium bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-md transition-all flex items-center justify-center gap-1"
                    >
                      <Icons.Edit /> Edit
                    </button>
                    <button
                      ...
                      className="px-2 py-1 text-xs font-medium bg-rose-100 dark:bg-rose-900/20 hover:bg-rose-200 dark:hover:bg-rose-900/40 text-rose-600 dark:text-rose-400 rounded-md transition-all flex items-center justify-center"
                    >
                      ✕
                    </button>
                  </div>
