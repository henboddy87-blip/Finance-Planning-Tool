...
function generateRetirementProjection({
  currentAge,
  retireAge,
  currentSavings,
  monthlyContrib,
  ...
  for (let age = currentAge; age <= 90; age++) {
    ...
    for (let m = 0; m < 12; m++) {
      if (!isRetired) {
        ...
        realBalance =
          realBalance * (1 + monthlyReturn - monthlyInflation) + monthlyContrib;
      } else {
        const withdrawal = 5000;
        balance = Math.max(0, balance * (1 + monthlyReturn) - withdrawal);
        realBalance = Math.max(
          0,
          realBalance * (1 + monthlyReturn - monthlyInflation) - withdrawal,
        );
      }
    }
    data.push({
...
export default function RetirementPlanner() {
  ...
  const investmentGrowth =
    retireBalance - params.currentSavings - totalContribs;

  const update = (k, v) =>
    setParams((p) => ({ ...p, [k]: parseFloat(v) || 0 }));

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      <PageHeader
        title={t("Retirement Planner")}
        subtitle={t("Model your path to financial independence")}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="relative bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:scale-[1.02] transition-all duration-200 overflow-hidden">
          <div
            ...
            style={{
              background: `radial-gradient(circle at top right, ${onTrack ? "#10b981" : "#f43f5e"}20, transparent 70%)`,
            }}
          />
          ...
          <div
            className={`text-2xl md:text-3xl font-bold ${onTrack ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
          >
            {fmt(retireBalance)}
          </div>
          <span
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={data}>
              ...
              <ReferenceLine
                ...
                stroke="#f59e0b"
                strokeDasharray="6 3"
                label={{ value: "Retire", fill: "#f59e0b", fontSize: 10 }}
              />
              <ReferenceLine
                y={target}
                stroke="#f43f5e"
            ...
            {[
              ...
              <div key={key}>
                <div className="flex justify-between mb-1">
                  <label className="text-xs text-gray-600 dark:text-gray-500 font-medium">
                    {label}
                  </label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:scale-[1.02] transition-all duration-200">
          <div className="flex items-center gap-2 mb-3">
            <Icons.Income />
            <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-500">
              Total Contributions
            </div>
          </div>
          ...
          <div className="text-xs text-gray-500 dark:text-gray-500">
            {yearsToRetire} yrs × {fmt(params.monthlyContrib * 12)}/yr
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:scale-[1.02] transition-all duration-200">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-500">
            {params.annualReturn}% annual return
          </div>
        </div>
