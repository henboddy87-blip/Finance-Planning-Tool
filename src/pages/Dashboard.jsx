...
export default function Dashboard({ onNavigate }) {
  ...
  const totalLiabilitiesValue = realLiabilities.reduce(
    (s, l) => s + l.value,
    0,
  );
  ...
  const allocationData = React.useMemo(() => {
    ...
    const map = {};
    const total = realHoldings.reduce(
  ...
  const totalOut =
    realPeriodTotalOut > 0
      ? realPeriodTotalOut
      : ieData.reduce((s, d) => s + d.expenses, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {t("Complete analytics overview · April 2026")}
          </p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 max-w-md w-full animate-fadeIn shadow-2xl">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
              Toggle widgets to show or hide on your dashboard.
            </p>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {ALL_WIDGETS.map((w) => (
                ...
                >
                  <span
                    className={`text-sm ${has(w.id) ? "text-gray-900 dark:text-gray-200" : "text-gray-600 dark:text-gray-400"}`}
                  >
                    {w.label}
                  </span>
                  <div
      ...
      {(has("cashflow") || has("networth")) && (
        <div
          className={`grid gap-5 mb-5 ${has("cashflow") && has("networth") ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}
        >
          {has("cashflow") && (
      ...
      {(has("savingsRate") || has("spending")) && (
        <div
          className={`grid gap-5 mb-5 ${has("savingsRate") && has("spending") ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}
        >
          {has("savingsRate") && (
            <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <div className="mb-3">
              ...
              <div className="mb-4">
                <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(avgSR, 100)}%` }}
                  />
                </div>
                <div className="flex justify-between text-xs text-gray-500 dark:text-gray-500 mt-1">
                  <span>0%</span>
                  <span className="text-amber-600 dark:text-amber-400 flex items-center gap-1">
                    <ArrowUp className="w-3 h-3" /> 20% min
                  </span>
                  <span>50%</span>
                </div>
              </div>
