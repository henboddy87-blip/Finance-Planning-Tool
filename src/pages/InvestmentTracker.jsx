...
export default function InvestmentTracker() {
  ...
  const annualDiv = holdings.reduce(
    (s, h) => s + h.shares * h.currentPrice * (h.divYield / 100),
    0,
  );
  const portfolioBeta = holdings.reduce(
    (s, h) => s + ((h.shares * h.currentPrice) / portfolioValue) * h.beta,
    0,
  );
  ...
  const allocationData = useMemo(() => {
    ...
    return Object.entries(map).map(([name, value]) => ({
      ...
      pct: ((value / portfolioValue) * 100).toFixed(1),
    }));
  ...
  const riskBreakdown = useMemo(() => {
    ...
    holdings.forEach((h) => {
      map[h.risk] = (map[h.risk] || 0) + h.shares * h.currentPrice;
    });
    return Object.entries(map).map(([name, value]) => ({
      name,
      value: Math.round(value),
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortBy === "risk")
      return (
        ["Low", "Medium", "High"].indexOf(b.risk) -
        ["Low", "Medium", "High"].indexOf(a.risk)
      );
  ...
  const handleAdd = () => {
    ...
    toast.success(
      "Holding Added",
      `Successfully added ${form.ticker.toUpperCase()}`,
    );
  };

  const TABS = ["portfolio", "performance", "risk", "allocation"];

  const techConcentration = (
    (holdings
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          trend={parseFloat(gainPct)}
          color="emerald"
        />
        <StatCard
          label={t("Total Gain/Loss")}
          value={fmt(totalGain)}
          sub={`${gainPct}% ${t("overall")}`}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 max-w-lg w-full animate-fadeIn shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                {
                  label: t("Shares / Units"),
                  key: "shares",
                  placeholder: t("10"),
                  type: "number",
                },
                {
                  label: t("Avg Cost / Share ($)"),
                  key: "avgCost",
                  placeholder: t("150.00"),
                  type: "number",
                },
                {
                  label: t("Current Price ($)"),
                  key: "currentPrice",
                  placeholder: t("180.00"),
                  type: "number",
                },
                {
                  label: t("Beta"),
      ...
      {tab === "portfolio" && (
        <>
          <div className="flex justify-between items-center mb-3 flex-wrap gap-3">
            ...
            <select
              className="px-2 py-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-xs text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500"
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
            >
              <option value="value">{t("Sort: Value")}</option>
              <option value="gain">{t("Sort: Gain%")}</option>
              <option value="risk">{t("Sort: Risk")}</option>
              <option value="ticker">{t("Sort: Ticker")}</option>
            </select>
          </div>
                    ...
                    return (
                      ...
                      >
                        <td className="p-4">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-500 max-w-[140px] truncate">
                            {h.name}
                          </div>
                        </td>
                        <td className="p-4">
                          <span
                            ...
                            style={{
                              background: `${CAT_COLORS[h.category]}20`,
                              color: CAT_COLORS[h.category],
                            }}
                          >
                            {h.category}
                          </span>
                        ...
                        <td className="p-4 text-sm text-gray-700 dark:text-gray-300">
                          {fmtFull(h.currentPrice)}
                        </td>
                        <td className="p-4 text-sm font-semibold text-gray-900 dark:text-gray-200">
                          {fmt(value)}
                        </td>
                        <td
                          className={`p-4 text-sm font-semibold ${gain >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
                        >
                          {gain >= 0 ? "+" : ""}
                          {fmt(gain)}
                        </td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-1 text-xs rounded-full ${gain >= 0 ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" : "bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400"}`}
                          >
                            {fmtPct(pct)}
                          </span>
                        </td>
                        ...
                        <td className="p-4">
                          <span
                            className="px-2 py-1 text-xs rounded-full"
                            style={{
                              background: `${RISK_COLOR[h.risk]}20`,
                        ...
                        <td className="p-4">
                          <button
                            onClick={() => {
                              ...
                              toast.success("Deleted", "Holding removed.");
                            }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div className="grid grid-cols-3 gap-2 mt-4">
              {[
                ...
                <div
                  key={i}
                  className="p-2 bg-gray-100 dark:bg-gray-900 rounded-lg text-center"
                >
          ...
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div className="space-y-2 mt-4">
              {[
                { label: t("Annual Dividend Income"), value: fmt(annualDiv) },
                {
                  label: t("Dividend Yield (Portfolio)"),
                  value: `${((annualDiv / portfolioValue) * 100).toFixed(2)}%`,
                },
                { label: t("Highest Yielder"), value: "VNQ (4.2%)" },
                { label: t("Next Ex-Div Date"), value: t("May 15, 2026") },
              ].map((r, i) => (
                <div
                  key={i}
                  className="flex justify-between text-sm py-1 border-b border-gray-100 dark:border-gray-700/50 last:border-0"
                >
              ...
              <PieChart>
                ...
                >
                  {allocationData.map((e, i) => (
                    <Cell key={i} fill={Object.values(CAT_COLORS)[i % 6]} />
                  ))}
                </Pie>
                <Tooltip
                  formatter={(v) => fmt(v)}
              ...
              {allocationData.map((d, i) => (
                <div key={i} className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span
                      className="w-2 h-2 rounded-sm"
                      style={{ background: Object.values(CAT_COLORS)[i % 6] }}
                    />
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-20 h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${d.pct}%`,
                          background: Object.values(CAT_COLORS)[i % 6],
                        }}
                      />
                    </div>
          ...
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div className="mt-4 p-3 bg-gray-100 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2 mb-2">
                <Icons.Warning />
                <div className="text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase tracking-wider">
                  {t("Concentration Warning")}
                </div>
              </div>
              <div className="text-sm text-amber-600 dark:text-amber-400">
                <AlertTriangle className="w-4 h-4 inline mr-1" /> Technology
                sector represents {techConcentration}% of your portfolio.
