...
export default function TaxCalculator() {
  ...
  const calc = useMemo(() => {
    ...
    const itemizedDed =
      Math.min(deductions.stateLocal, 10000) +
      deductions.mortgage +
    ...
    const ficaTax =
      Math.min(income.wages, 168600) * 0.062 + income.wages * 0.0145;
    const seTax = income.selfEmployed * 0.9235 * 0.153;
    const totalTax =
      fedTax + stateTax + Math.round(ficaTax) + Math.round(seTax);
    ...
    return {
      ...
      itemizedDed,
      chosenDed,
      marginalRate: marginalBracket ? marginalBracket.rate * 100 : 0,
    };
  ...
  const pieData = [
    ...
    {
      name: "Net Income",
      value: Math.max(0, calc.grossIncome - calc.totalTax),
      color: "#10b981",
    },
  ];

  const updateIncome = (k, v) =>
    setIncome((p) => ({ ...p, [k]: parseFloat(v) || 0 }));
  const updateDed = (k, v) =>
    setDeductions((p) => ({ ...p, [k]: parseFloat(v) || 0 }));

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      <PageHeader
        title={t("Tax Calculator")}
        subtitle={t("Estimate your 2024 federal and state income taxes")}
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-900/20 dark:to-emerald-900/5 backdrop-blur-sm rounded-xl border border-emerald-200 dark:border-emerald-500/30 p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-gray-500 mb-2">
            Gross Income
          </div>
        ...
        <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
            ...
            <div className="flex gap-2 items-center">
              ...
              <div
                onClick={() => setUseItemized((p) => !p)}
                className={`w-9 h-5 rounded-full cursor-pointer transition-all ${useItemized ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`}
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white transition-all mt-0.5 ${useItemized ? "ml-4" : "ml-0.5"}`}
                />
              </div>
            </div>
          </div>
          <div
            className={`p-2 bg-gray-50 dark:bg-gray-900 rounded-lg mb-3 border ${!useItemized ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-500/50" : "border-gray-300 dark:border-gray-700"}`}
          >
            <div className="text-xs text-gray-600 dark:text-gray-500 mb-1">
              Standard Deduction
            </div>
            <div
              className={`font-semibold ${!useItemized ? "text-emerald-700 dark:text-emerald-400" : "text-gray-500 dark:text-gray-500"}`}
            >
              {fmt(calc.standardDed)}
            </div>
          </div>
          <div
            className={`p-2 bg-gray-50 dark:bg-gray-900 rounded-lg mb-3 border ${useItemized ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-500/50" : "border-gray-300 dark:border-gray-700"}`}
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={key}>
                <label
                  className={`block text-xs font-medium mb-1 ${useItemized ? "text-gray-600 dark:text-gray-500" : "text-gray-400 dark:text-gray-600"}`}
                >
                  {label}
                </label>
                <input
                  className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500 transition-colors disabled:opacity-50 disabled:bg-gray-100 dark:disabled:bg-gray-800"
                  type="number"
                  value={deductions[key]}
                  onChange={(e) => updateDed(key, e.target.value)}
                  disabled={!useItemized}
                />
              </div>
            ))}
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRetirement401k(parseFloat(e.target.value) || 0)
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                type="number"
                value={hsaContrib}
                onChange={(e) => setHsaContrib(parseFloat(e.target.value) || 0)}
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          <div className="space-y-2 mb-4">
            {[
              ...
              {
                label: `Less: ${useItemized ? "Itemized" : "Standard"} Deduction`,
                value: -calc.chosenDed,
                color: "text-emerald-600 dark:text-emerald-400",
              },
              {
                label: "Less: Pre-tax Contribs",
                value: -(retirement401k + hsaContrib),
                color: "text-emerald-600 dark:text-emerald-400",
              },
              {
              ...
              {
                label: "State Tax (est. 5.5%)",
                value: calc.stateTax,
                color: "text-rose-600 dark:text-rose-400",
              },
              {
                label: "FICA / SE Tax",
                value: calc.ficaTax + calc.seTax,
              ...
              null,
              {
                ...
                value: calc.grossIncome - calc.totalTax,
                color: "text-emerald-600 dark:text-emerald-400",
                bold: true,
              },
              {
                label: "Monthly Take-home",
                value: (calc.grossIncome - calc.totalTax) / 12,
                color: "text-emerald-600 dark:text-emerald-400",
              },
            ].map((item, i) =>
              item === null ? (
                ...
                <div
                  key={i}
                  className={`flex justify-between ${item.bold ? "text-sm" : "text-xs"}`}
                >
          ...
          <table className="w-full">
            ...
            <tbody>
              {calc.breakdown.map((b, i) => (
                ...
                >
                  <td className="p-4">
                    <span className="px-2 py-1 text-xs rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400">
                      {b.rate}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-sm font-semibold text-rose-600 dark:text-rose-400">
                    {fmt(b.tax)}
                  </td>
                  <td className="p-4 text-sm text-gray-500 dark:text-gray-500">
                    {calc.grossIncome > 0
                      ? ((b.tax / calc.grossIncome) * 100).toFixed(1)
                      : 0}
                    %
                  </td>
                </tr>
