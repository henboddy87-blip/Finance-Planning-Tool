...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-3 shadow-2xl ring-1 ring-black/5 dark:ring-white/5 animate-scaleIn min-w-[140px]">
      {label && (
        <div className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 border-b border-gray-100 dark:border-gray-800/50 pb-1.5">
          {label}
        </div>
      )}
          ...
          <div key={i} className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ background: p.color || p.payload?.fill || "#555" }}
              />
...
export default function FamilyMode() {
  ...
  const [inviteForm, setInvForm] = useState({
    name: "",
    email: "",
    role: "Member",
    income: "",
  ...
  const handleInvite = () => {
    ...
    const newMember = {
      ...
      income: incomeVal,
      incomeSources: { Salary: incomeVal, Freelance: 0, Passive: 0, Other: 0 },
      expenses: 0,
      budgetShare: budgetShareVal,
      joined: now,
      lastActive: "Just now",
      permissions:
        inviteForm.role === "Admin"
          ? { view: true, edit: true, invite: true, delete: true }
          : inviteForm.role === "Member"
            ? { view: true, edit: true, invite: false, delete: false }
            : { view: true, edit: false, invite: false, delete: false },
    };
    ...
    toast.success(
      "Member Added",
      `${inviteForm.name} has been added to the family group`,
    );
  };

  const handleSettle = (id) => {
    setSplits((prev) =>
      prev.map((s) => (s.id === id ? { ...s, settled: true } : s)),
    );
  ...
  const handleAddSplit = () => {
    ...
    const newSplit = {
      ...
      split: members.reduce(
        (acc, m) => ({
          ...acc,
          [m.name.split(" ")[0]]: parseFloat(perPerson.toFixed(2)),
        }),
        {},
      ),
    };
    ...
    toast.success(
      "Split Created",
      `Expense for "${splitForm.desc}" split successfully.`,
    );
  };
  ...
  const handleAddBudget = () => {
    ...
    toast.success(
      "Budget Created",
      `Shared budget "${budgetForm.name}" created with ${newBudget.members.length} members.`,
    );
  };
  ...
  const handleUpdateProgress = () => {
    ...
    toast.success(
      "Progress Updated",
      `Successfully added ${fmt(val)} to "${updateBudget.name}".`,
    );
  };
  ...
  const updateMemberIncome = (id, income) => {
    setMembers((prev) =>
      prev.map((m) =>
        m.id === id ? { ...m, income: parseFloat(income) || 0 } : m,
      ),
    );
  ...
  const handleConfirmRemove = () => {
    ...
    toast.success(
      "Member Removed",
      `${name} has been removed from the family group.`,
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 animate-fadeIn">
      <PageHeader
        title={t("Family Mode")}
        subtitle={t("Shared finances, expense splitting and family budgeting")}
        actions={
          <button
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setInvForm((p) => ({ ...p, name: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setInvForm((p) => ({ ...p, email: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setInvForm((p) => ({ ...p, role: e.target.value }))
                  }
                >
                  {ROLES.map((r) => (
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      placeholder="5000"
                      value={inviteForm.income}
                      onChange={(e) =>
                        setInvForm((p) => ({ ...p, income: e.target.value }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      placeholder="500"
                      value={inviteForm.budgetShare}
                      onChange={(e) =>
                        setInvForm((p) => ({
                          ...p,
                          budgetShare: e.target.value,
                        }))
                      }
                    />
                  </div>
                ...
                <div className="text-[10px] text-gray-500 dark:text-gray-500 mt-2 p-2 bg-emerald-50 dark:bg-emerald-900/10 rounded-lg border border-emerald-200 dark:border-emerald-800 leading-relaxed">
                  ...
                  sets this member's monthly earnings.{" "}
                  <strong className="text-emerald-700 dark:text-emerald-400">
                    Budget Share
                  </strong>{" "}
                  is how much they contribute to shared family expense budgets
                  each month.
                </div>
              </div>
      ...
      <div className="flex gap-0 mb-5 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
        {TABS.map((tabKey) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium capitalize transition-all border-b-2 whitespace-nowrap ${
              tab === tabKey
          ...
          {members.length === 0 ? (
            <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 max-w-sm mx-auto">
                Start by adding your first family member. You can set their
                income, role, and budget sharing amount.
              </p>
              <button
                onClick={() => setInvite(true)}
                className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-all flex items-center gap-2 mx-auto"
              >
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                  <div className="text-sm font-semibold text-gray-900 dark:text-gray-200 mb-4">
                    Member Contribution
                  </div>
                ...
                <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 flex flex-col">
                  <div className="text-sm font-semibold text-gray-900 dark:text-gray-200 mb-4 flex justify-between items-center">
                    ...
                    <span className="text-[10px] bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded-full font-medium">
                      Monthly
                    </span>
                  </div>
                  <div className="flex-1 flex flex-col sm:flex-row items-center gap-6">
                    <div className="relative w-full sm:w-1/2 aspect-square max-w-[180px]">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <div className="text-[10px] text-gray-500 dark:text-gray-500 font-medium uppercase tracking-tighter">
                          Total
                        </div>
                        <div className="text-lg font-bold text-gray-900 dark:text-white leading-tight">
                          {fmt(totalFamilyExpenses)}
                        </div>
                      </div>
                    </div>
                      ...
                      {members.map((m, i) => {
                        const share =
                          totalFamilyExpenses > 0
                            ? (m.expenses / totalFamilyExpenses) * 100
                            : 0;
                        return (
              ...
              <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {members.map((m) => (
                    <div
                      key={m.id}
                      className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700"
                    >
                      <div
                  ...
                  >
                    ...
                    <td className="p-4">
                      <select
                        ...
                        className="px-2 py-1 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-xs text-gray-900 dark:text-gray-200 focus:outline-none focus:border-emerald-500"
                      >
                        {ROLES.map((r) => (
                          <option key={r}>{r}</option>
                        ))}
                    ...
                    <td className="p-4 text-sm text-rose-600 dark:text-rose-400">
                      {fmt(m.expenses)}
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex gap-1">
                        <button
                          ...
                          className="p-1.5 text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
                        >
                          <Icons.Edit />
                        </button>
                        {m.id !== 1 && (
                          <button
                            ...
                            className="p-1.5 text-rose-500 dark:text-rose-400 hover:text-rose-600 dark:hover:text-rose-500 transition-colors"
                          >
                            <Icons.Remove />
                          </button>
                        )}
      ...
      {tab === "budgets" && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                Total: {fmt(totalFamilyIncome)}/mo
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {members.map((m) => {
                ...
                const pctOfTotal =
                  totalFamilyIncome > 0
                    ? ((m.income / totalFamilyIncome) * 100).toFixed(0)
                    : 0;
                return (
                  <div
                    key={m.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <button
                          onClick={() =>
                            setIncomeEditing(isExpanded ? null : m.id)
                          }
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-200 uppercase tracking-wider">
                Shared Expense Budgets
              </h3>
              <button
                onClick={() => setShowAddBudget(true)}
                className="px-3 py-1.5 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-all flex items-center gap-1"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {budgets.map((b) => {
                ...
                return (
                  <div
                    key={b.id}
                    className="relative bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:border-gray-300 dark:hover:border-gray-600 transition-all overflow-hidden"
                  >
                    <div
                    ...
                    <div className="pl-2">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1 min-w-0 pr-2">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {b.category && (
                              <span className="px-1.5 py-0.5 text-[9px] rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 shrink-0">
                                {b.category}
                              </span>
                            )}
                          </div>
                            ...
                            {members.map((m) => {
                              ...
                              return (
                                <button
                                  key={m.id}
                                  onClick={() =>
                                    toggleBudgetMember(b.id, m.avatar)
                                  }
                                  title={
                                    isIncluded
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <button
                            onClick={() => {
                              const isMember = b.members.includes(
                                user?.initials || "AJ",
                              );
                              ...
                              setBudgets((prev) =>
                                prev.map((bud) => {
                                  ...
                                  const newMembers = isMember
                                    ...
                                    : [...bud.members, memberAvatar];
                                  const newContribs = { ...bud.contributions };
                                  if (isMember) {
                                    delete newContribs[memberAvatar];
                                  } else {
                                    newContribs[memberAvatar] = 0;
                                  }
                                  return {
                                    ...bud,
                                    members: newMembers,
                              ...
                              toast.info(
                                isMember ? "Left Budget" : "Joined Budget",
                                isMember
                                  ? "You have left this shared budget."
                                  : "You have joined this shared budget.",
                              );
                            }}
                      ...
                      <div className="flex justify-between items-end mb-1">
                        <span
                          ...
                          style={{ color: over ? "#f43f5e" : b.color }}
                        >
                          {fmt(b.isGoal ? b.saved : b.spent)}
                        </span>
                      ...
                      <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mb-2">
                        <div
                          ...
                          style={{
                            ...
                            background: over ? "#f43f5e" : b.color,
                          }}
                        />
                      </div>
                      ...
                      {b.contributions &&
                        Object.keys(b.contributions).length > 0 && (
                          <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700/50">
                            <div className="text-[10px] text-gray-500 dark:text-gray-500 mb-1 font-medium">
                              Member Contributions
                            </div>
                            <div className="space-y-1">
                              {Object.entries(b.contributions).map(
                                ([avatar, amount]) => {
                                  const member = members.find(
                                    (m) => m.avatar === avatar,
                                  );
                                  const contribPct =
                                    b.total > 0 ? (amount / b.total) * 100 : 0;
                                  return member ? (
                                    <div
                                      key={avatar}
                                      className="flex items-center gap-2"
                                    >
                                      <span className="text-[9px] text-gray-500 dark:text-gray-400 w-12 truncate">
                                        {member.name.split(" ")[0]}
                                      </span>
                                      ...
                                      <span className="text-[9px] text-gray-600 dark:text-gray-300 w-12 text-right font-medium">
                                        {fmt(amount)}
                                      </span>
                                    </div>
                                  ) : null;
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-[10px] text-gray-600 dark:text-gray-500">
                          {pct.toFixed(0)}% {b.isGoal ? "saved" : "used"} ·{" "}
                          {b.members.length} members
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBudgetForm((p) => ({ ...p, name: e.target.value }))
                  }
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBudgetForm((p) => ({ ...p, total: e.target.value }))
                  }
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Share with Members & Allocate Contribution
                </label>
                <div className="space-y-3">
                  {members.map((m) => {
                    const isSelected = budgetForm.selectedMembers.includes(
                      m.avatar,
                    );
                    return (
                      <div
                        key={m.id}
                        className={`p-3 rounded-xl border transition-all ${isSelected ? "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500" : "bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700"}`}
                      >
                        <div className="flex items-center justify-between">
                          ...
                          >
                            ...
                            <div className="text-left">
                              <div
                                className={`text-sm font-semibold ${isSelected ? "text-emerald-700 dark:text-emerald-400" : "text-gray-700 dark:text-gray-400"}`}
                              >
                                {m.name.split(" ")[0]}
                              </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  placeholder="0.00"
                                  value={
                                    budgetForm.memberAmounts[m.avatar] || ""
                                  }
                                  onChange={(e) =>
                                    setBudgetForm((p) => ({
                                      ...p,
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                <div
                  onClick={() =>
                    setBudgetForm((p) => ({ ...p, isGoal: !p.isGoal }))
                  }
                  className={`w-9 h-5 rounded-full cursor-pointer transition-all ${budgetForm.isGoal ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`}
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-all mt-0.5 ${budgetForm.isGoal ? "ml-4" : "ml-0.5"}`}
                  />
                </div>
              ...
              <div className="p-3 bg-gray-100 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
                ...
                <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                  {fmt(
                    splits
                      .filter(
                        (s) => !s.settled && s.paidBy !== currentUserFirstName,
                      )
                      .reduce(
                        (sum, s) => sum + (s.split[currentUserFirstName] || 0),
                        0,
                      ),
                  )}
                </div>
                ...
                <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">
                  {fmt(
                    splits
                      ...
                      .reduce(
                        (sum, s) =>
                          sum + s.amount - (s.split[currentUserFirstName] || 0),
                        0,
                      ),
                  )}
                </div>
              ...
              <table className="w-full min-w-[640px]">
                ...
                <tbody>
                  {splits.map((s) => (
                    <tr
                      key={s.id}
                      className={`border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors ${s.settled ? "opacity-60" : ""}`}
                    >
                      ...
                      <td className="p-4">
                        {!s.settled && (
                          <button
                            ...
                            className="px-2 py-1 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-md transition-all flex items-center gap-1"
                          >
                            <Icons.Settle /> Settle
                          </button>
                        )}
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  value={splitForm.desc}
                  onChange={(e) =>
                    setSplitForm((p) => ({ ...p, desc: e.target.value }))
                  }
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSplitForm((p) => ({ ...p, paidBy: e.target.value }))
                  }
                >
                  {members.map((m) => (
              ...
              <div className="p-3 bg-emerald-50 dark:bg-emerald-900/10 rounded-lg border border-emerald-200 dark:border-emerald-800">
                ...
                <div className="text-[10px] text-emerald-600 dark:text-emerald-500 mt-1">
                  Amount will be divided equally among all {members.length}{" "}
            ...
            <table className="w-full min-w-[640px]">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="text-center p-4 text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase">
                    View Data
                  </th>
                  <th className="text-center p-4 text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase">
                    Edit
                ...
                {members.map((m) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-center">
                      <span
                        className={`text-base font-bold ${m.role !== "Viewer" ? "text-emerald-600 dark:text-emerald-400" : "text-gray-300 dark:text-gray-600"}`}
                      >
                        {m.role !== "Viewer" ? "✓" : "✕"}
                      </span>
                    </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 max-w-sm w-full animate-fadeIn shadow-2xl">
            <div className="text-center mb-4">
              ...
              <p className="text-xs text-rose-500 dark:text-rose-400 mt-2">
                This action cannot be undone. All shared budgets and splits
                involving this member will be updated.
              </p>
            </div>
            <div className="flex gap-3">
              <button
                className="flex-1 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-medium rounded-lg transition-all"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 max-w-sm w-full animate-fadeIn shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                Add {updateBudget.isGoal ? "Savings" : "Expense"}
              </h3>
              <button
