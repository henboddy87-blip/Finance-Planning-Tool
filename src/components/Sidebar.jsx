...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { id: "networth", labelKey: "Net Worth", icon: CircleDollarSign },
      { id: "reports", labelKey: "Reports & Export", icon: FileBarChart },
    ],
  },
  {
    labelKey: "Planning",
    items: [
      { id: "budget", labelKey: "Budget Planner", icon: PieChart },
      { id: "goals", labelKey: "Goals", icon: Zap },
      { id: "retirement", labelKey: "Retirement", icon: Building2 },
      { id: "debt", labelKey: "Debt Manager", icon: FileStack },
      { id: "calculators", labelKey: "Calculators", icon: Calculator },
    ],
  },
  {
    labelKey: "Money",
    items: [
      { id: "investments", labelKey: "Investments", icon: TrendingUp },
      { id: "bank", labelKey: "Bank & Payments", icon: CreditCard },
      { id: "bills", labelKey: "Bills & Subs", icon: CalendarDays },
      { id: "tax", labelKey: "Tax Calculator", icon: Receipt },
    ],
  },
  {
    labelKey: "Account",
    items: [
      { id: "family", labelKey: "Family Mode", icon: Users },
...
export default function Sidebar({ currentPage, onNavigate, isOpen, onToggle }) {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen flex flex-col z-[100] transition-all duration-300 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 ${isOpen ? "w-[260px]" : "w-[72px]"}`}
    >
      {/* Logo row */}
      <div className="flex items-center gap-3 px-5 min-h-[68px] shrink-0 border-b border-gray-100 dark:border-gray-800">
        ...
        {isOpen && (
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-gray-800 dark:text-gray-100 text-base leading-none whitespace-nowrap tracking-tight">
              Smart Money
            </div>
            <div className="text-[10px] font-bold tracking-widest mt-0.5 uppercase text-gray-400 dark:text-gray-600">
              Financial Planner
            </div>
          </div>
        )}
        <button
          onClick={onToggle}
          className="ml-auto p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 transition-colors shrink-0"
        >
          <X className="w-5 h-5 md:hidden" />
          <ChevronsLeft
            className={`hidden md:block w-4 h-4 transition-transform duration-300 ${!isOpen ? "rotate-180" : ""}`}
          />
        </button>
      </div>
          ...
          <div key={gi} className="mb-4">
            ...
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  {active && (
                    <div className="absolute left-0 w-0.5 h-6 bg-emerald-500 rounded-r-full" />
                  )}
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex flex-col gap-2">
        <button
          onClick={() =>
            updateUser({
              language: user?.language === "Khmer" ? "English" : "Khmer",
            })
          }
      ...
      {isOpen && (
        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-gray-800 dark:text-gray-100 truncate leading-tight">
                {user?.name || "Peter Son"}
              </div>
              <div className="text-[11px] font-medium text-emerald-600 dark:text-emerald-400 mt-0.5 uppercase tracking-wide">
                Financial Member
              </div>
            </div>
          </div>
